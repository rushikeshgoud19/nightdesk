...
integrity pass closed.

The design document this is derived from is
`NIGHTDESK_GAME_PLAN_README.md` — 29 sections of design, none of it sequenced.
This file sequences it. One section at a time, each leaving a playable game.
...
| Persistence | ProfileStore round trip proven against place `106512529474987` |
| Art | Materials + lighting pass landed |
...
Discharges: nothing new. Repairs §19, §20.

### Section 1 — The Night — §15, §16, §17
...
**First because** every later system needs a schedule to happen on. Motel events
need a clock. Difficulty needs phases. Procedural nights need a seed.

### Section 2 — The Shared Desk — §13, §14
...
**Second because it is architecture, not content.** Every section after it lands
co-op-native instead of needing its own port. Doing this last would mean porting
six systems instead of one.

**Known risk:** `GuestRenderer` draws from unseeded `math.random` in 15 places.
Idle jitter can diverge between clients harmlessly, but the facial asymmetry
offset cannot — the guest's *build* has to be a pure function of the wire
payload. Settle that first; it decides whether this section is cheap or hard.

### Section 3 — Evidence Chains — §6, §7
...
### Section 5 — Something Got Inside — §5.5, §5.6, §8, §9, §11
...
### Section 6 — Unreliable Sanity — §10
...
**After 3 and 5 because it works by corrupting them.** Before them there is
nothing to corrupt.

### Section 7 — Confront / Defend — §12

The survival verb. Stun, interrupt, buy time, protect a teammate. Never a kill
button.

### Section 8 — Economy & Upgrade Depth — §19, §20
...
Halloween 2026 is about eight weeks out. Nine sections do not fit eight weeks at
a comfortable pace.

**Must ship:** 1, 2, 3, 4, 5, 8.
**Cut first if time runs out:** 7, then 6.

Section 7 is the one the design itself warns against overusing (§12, §23), so it
...
| `asymOffset()` never returns 0 | a genuine face rendering perfectly symmetric is the `perfect_symmetry` anomaly |
